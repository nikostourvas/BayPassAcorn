...
rule all:
    input:
        ...
        expand("results/WZA_res/{sample}_{envfactor}_BF_WZA_output_{chunk}.csv", 
            sample=SAMPLES, envfactor=ENVFACTOR_NAMES, chunk=range(1, WZA_CHUNKS+1)),
        expand("results/WZA_res/{sample}_{envfactor}_spearman_WZA_output_{chunk}.csv",
            sample=SAMPLES, envfactor=ENVFACTOR_NAMES, chunk=range(1, WZA_CHUNKS+1)),
