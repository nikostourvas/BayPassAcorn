...
#rule all:
#    input:
#        expand("results/{sample}.genobaypass.sub{i}", sample=SAMPLES,
#            i=range(1, N_SUBS+1))

rule all:
    input:
        expand("results/{sample}_concatenated_res_core.csv", sample=SAMPLES),
        expand("results/{sample}_concatenated_res_std.csv", sample=SAMPLES),
...
rule vcf2genobaypass:
    ...
    params:
        ...
        subs=N_SUBS,
    output:
        genobaypass=["results/{{sample}}.genobaypass.sub{}".format(i) for i in range(1, N_SUBS+1)],
        snpdet=["results/{{sample}}.snpdet.sub{}".format(i) for i in range(1, N_SUBS+1)]
...
rule run_baypass_1:
    ...
    params:
        ...
        npilot=20
    output:
        dic = "results/{sample}_baypassSplitOut_core/core_{i}_DIC.out",
        mat_omega = "results/{sample}_baypassSplitOut_core/core_{i}_mat_omega.out",
        summary_beta_params = "results/{sample}_baypassSplitOut_core/core_{i}_summary_beta_params.out",
        summary_lda_omega = "results/{sample}_baypassSplitOut_core/core_{i}_summary_lda_omega.out",
        summary_pi_xtx = "results/{sample}_baypassSplitOut_core/core_{i}_summary_pi_xtx.out",
        summary_yij_pij = "results/{sample}_baypassSplitOut_core/core_{i}_summary_yij_pij.out"
    shell:
        """
        g_baypass \
...
rule run_baypass_3:
    ...
    output:
        ...
        mat_omega = "results/{sample}_baypassSplitOut_contrast/contrast_{i}_mat_omega.out",
        ...
        summary_lda_omega = "results/{sample}_baypassSplitOut_contrast/contrast_{i}_summary_lda_omega.out",
...
rule concatenate_results:
    input:
        core = expand("results/{{sample}}_baypassSplitOut_core/core_{i}_summary_beta_params.out", i=range(1, N_SUBS+1)),
        std = expand("results/{{sample}}_baypassSplitOut_std/std_{i}_summary_betai_reg.out", i=range(1, N_SUBS+1)),
        contrast = expand("results/{{sample}}_baypassSplitOut_contrast/contrast_{i}_summary_betai_reg.out", i=range(1, N_SUBS+1))
    params:
        prefixcore = "results/{sample}_baypassSplitOut_core/core",
        prefixstd = "results/{sample}_baypassSplitOut_std/std",
        prefixcontrast = "results/{sample}_baypassSplitOut_contrast/contrast",
        snpdetprefix = "results/{sample}.snpdet.sub"
    output:
        coretable = "results/{sample}_concatenated_res_core.csv",
        stdtable = "results/{sample}_concatenated_res_std.csv",
